...
For the step-by-step configuration recipes, see [Set up authentication](../getting-started/authentication.mdx). This page explains *how* each mode works end-to-end.
...
2. **Infrahub layer** — controls *what* the MCP server can do in Infrahub. Configured via `INFRAHUB_API_TOKEN` (for `none` and `oidc` modes) or via the client's own token (for `token-passthrough` mode). Username/password authentication is not supported; use OIDC when you need external identity.
