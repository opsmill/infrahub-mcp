...
When either threshold is crossed, reads return a `Schema temporarily unavailable` error to the agent. The cache still tries to heal itself: a read against a tripped entry attempts one revalidation and only fails closed if that attempt also fails, and a success resets both counters. Those recovery probes are throttled to one every `min(INFRAHUB_MCP_SCHEMA_CACHE_TTL, 30)` seconds, so a *tripped* branch costs one upstream timeout per window rather than one per request. The throttle applies only once an entry is tripped: through the first failures of an outage, each read past the skip-window still attempts its own revalidation. The 30-second ceiling keeps recovery prompt even when the skip-window is set much wider to cut steady-state load.
