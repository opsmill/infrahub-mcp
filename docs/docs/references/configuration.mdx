...
The MCP server supports three authentication modes controlled by `INFRAHUB_MCP_AUTH_MODE`:
...
| 10 | **TokenPassthroughMiddleware** — fail-closed gate for token-passthrough mode | no |
| 11 | **ReadOnlyMiddleware** — tag-based write tool blocking | no |
| 12 | **AuditMiddleware** — structured audit trail for tool/resource access | yes |
| 13 | **ResponseCachingMiddleware** — TTL-based response caching | no |
| 14 | **DereferenceRefsMiddleware** — inline `$ref` in JSON schemas | no |
| 15 | **PingMiddleware** — periodic keepalive pings | no |
| 16 | **ResponseLimitingMiddleware** — truncates responses over 500 KB | yes |
