...
| `INFRAHUB_MCP_BRANCH_PROTECTED` | Comma-separated branch names that reject direct writes (route via `propose_changes`). Empty string disables. | `main` |
...
| 11 | **TokenPassthroughMiddleware** — fail-closed gate for token-passthrough mode | no |
