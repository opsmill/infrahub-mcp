...
| `INFRAHUB_MCP_ENV_FILE` | Path to a `.env` file whose `INFRAHUB_`-prefixed keys are loaded at startup (real env vars win). A missing default file is ignored; set to empty to disable loading | `./.env` |
