...
Schema changes made upstream are observed within `schema_cache_ttl + one /summary round-trip` of the change. Setting `INFRAHUB_MCP_SCHEMA_CACHE_ENABLED=false` reverts to the pre-feature baseline (full schema re-fetch on every request in passthrough modes).
