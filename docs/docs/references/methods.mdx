...
Find nodes by partial name. Matches substrings against the `name` attribute only (via `name__value` with `partial_match=true`). For other attributes, or combining filters, use `get_nodes` instead.
...
Execute a read-only GraphQL query against Infrahub. Mutations are rejected at the AST level — use `mutate_graphql` instead. For simple attribute reads, prefer `get_nodes` or `search_nodes`.
