...
Graph traversal walks the live data graph to find how objects connect. It requires Infrahub 1.10 or later. The `source` and `destination` arguments accept either a node UUID or a kind-qualified human-friendly ID (for example `InfraDevice__atl1-edge1`, the form `get_nodes` returns); unresolvable references return an error pointing at `get_nodes`.
