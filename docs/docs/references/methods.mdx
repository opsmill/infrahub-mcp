...
Response is encoded in **TOON** tabular format — arrays for `attributes`, `relationships`, and `filters` use the header-once layout.
...
- **include_attributes** (boolean, default: false): return full attribute values in TOON tabular format instead of just display labels; use when you need structured data, not just names
<!-- vale on -->
