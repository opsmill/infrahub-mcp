...
If you're connecting to a remote server over HTTP instead, replace the `command`/`args`/`env` block with a `url` field — see the [Docker Compose guide](../guides/docker-compose.mdx#connect-your-mcp-client) for the HTTP transport configuration.
...
| --- | --- | --- |
| Server won't start — `INFRAHUB_MCP_AUTH_MODE` validation error | Missing OIDC configuration or token-passthrough over stdio | Use `auth_mode=none` for stdio, or switch to `--transport streamable-http`. |
