...
| `INFRAHUB_API_TOKEN`| API token for Infrahub *(or use username + password)* | —           |
...
Add the MCP server configuration to your project-level or user-level `settings.json` file. For project-level, create or edit `.claude/settings.json` in your repository root. For user-level, edit `~/.claude/settings.json`.

Alternatively, add an `.mcp.json` file at the root of your project.
...
Claude Code automatically detects `.mcp.json` in the project root and loads MCP servers on startup.
