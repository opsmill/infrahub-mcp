...
When the MCP server runs over Streamable HTTP with `token-passthrough` or `basic-passthrough` auth, clients connect via URL and pass credentials in the `Authorization` header instead of using stdio with env vars. See [Set up authentication](./authentication.mdx) for server-side configuration.
