...
The server loads `.env` at startup into its environment. Values already set in
the real environment (including the `.mcp.json` `env:` block) take precedence,
so `.env` never overrides an explicitly set variable. To load a file from a
different location, set `INFRAHUB_MCP_ENV_FILE=/path/to/file`.
