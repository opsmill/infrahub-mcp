...
- Cache value: an immutable `CachedSchemaEntry` carrying the `BranchSchema`, the schema hash, the cached GraphQL SDL string (`None` when only that fetch failed — see below), the monotonic timestamp of the last successful fetch, a consecutive-revalidation-failure counter, and the timestamp of the last failed lazy SDL fill (see below).
