...
- **FR-008**: System MUST serve cached data and emit an operator-visible warning when *any* upstream failure occurs during revalidation or full refetch (including 4xx authentication errors, 5xx server errors, and network errors) and a previously cached entry is available; the differentiation between transient and definitive failures is not made at this layer because schema content is global per branch and serving a cached copy does not leak per-user-protected data.
  - *Amended 2026-09-04 (commits 95786e6, 1b5db83; see [ADR 0009](../../../dev/adr/0009-hash-validated-schema-cache.md))*: a rejected credential (401/403 from the summary probe, the full schema fetch, or the SDK's login/token refresh) is no longer served stale. It is raised to that caller as `AuthenticationError` and touches neither the consecutive-failure counter, the cold-failure marker, nor the SDL-fill throttle, because in passthrough modes it describes the caller's credential, not upstream health. Only a non-200 from `/schema.graphql` itself (the SDK's bare `ValueError`) stays transient.
  - *Amended 2026-09-09 (see [ADR 0009](../../../dev/adr/0009-hash-validated-schema-cache.md))*: the SDL is now fetched through `client._get` with a URL-encoded branch and `raise_for_status()`, like the summary probe, instead of the SDK's `get_graphql_schema`. A 401/403 answered by `/schema.graphql` itself is therefore also raised to the caller as `AuthenticationError` and arms nothing; the `ValueError` residual above no longer exists.
